...
from ManagerAgent.router_intelligence import classify_intent, IntentType, RouterDecision
...
async def test_classify_stock_intent():
    ...
    mock_json = '{"intent": "stock", "reasoning": "User asked for price"}'
...
async def test_classify_rag_intent():
    ...
    with patch("ManagerAgent.router_intelligence.completion") as mock_completion:
        ...
        assert decision.intent == IntentType.RAG

@pytest.mark.asyncio
async def test_classify_calculator_intent():
    """Test classification of calculator queries."""
    mock_json = '{"intent": "calculator", "reasoning": "User asking for tax math"}'
    ...
    with patch("ManagerAgent.router_intelligence.completion") as mock_completion:
        ...
        assert decision.intent == IntentType.CALCULATOR
...
async def test_classify_error_handling():
    ...
    with patch("ManagerAgent.router_intelligence.completion") as mock_completion:
        ...
        assert decision.intent == IntentType.GENERAL
