...
TEST_QUERIES = [
    ...
    (
        "Loan",
        "I have a $200,000 mortgage at 6.5% for 30 years. What's my monthly payment?",
    ),
    ("Tax", "What would my federal taxes be on $85,000 income filing single?"),
    # Complex query with noise
    (
        "TVM with fluff",
        "So I've been thinking about retirement lately. If I put $50,000 into an index fund today at 8% average returns, what would that be worth when I'm 65 in 30 years?",
    ),
...
async def test_wolfram_api():
    ...
    try:
        result = await query_wolfram(
            "compound interest 1000 dollars at 5% for 10 years"
        )
