...
TEST_CASES = [
    ...
    (
        "If I invest $1000 monthly for 10 years at 7%, what is the future value?",
...
async def test_router_accuracy(query, expected_intent):
    ...
    assert decision.primary_intent == expected_intent, (
        f"Failed for query '{query}'. Expected {expected_intent}, got {decision.primary_intent}. Reason: {decision.reasoning}"
    )
