...
TEST_DATA_PATH = os.path.join(os.path.dirname(__file__), "training_data", "FiQA-PhraseBank.csv")
...
def run_evaluation(sample_size=None):
    ...
    if incorrect:
        ...
        for r in incorrect[:8]:  # Show first 8
            ...
            print(f"  True: {r['true_label']:8s} | Pred: {r['pred_label']:8s} (conf: {r['confidence']:.2f})")
            print()
