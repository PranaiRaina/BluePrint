...
def wolfram_compute_volatility(prices: list) -> dict:
    ...
    if not WOLFRAM_AVAILABLE or not WOLFRAM_KEY_ID or not WOLFRAM_KEY_SECRET:
        ...
        print(
            "[WARN] Wolfram Cloud not configured. Using Python fallback for volatility calculation."
        )
...
def wolfram_risk_analysis(
    ticker: str, prices: list = None, metrics: dict = None, analyst_ratings: dict = None
) -> dict:
    """
    ...
    if prices and len(prices) >= 10:
        ...
        volatility_result = {
            "error": "Insufficient price data for volatility calculation"
        }
    ...
    if analyst_ratings and "error" not in analyst_ratings:
        ...
        result["buyCount"] = analyst_ratings.get("buy", 0) + analyst_ratings.get(
            "strongBuy", 0
        )
        result["sellCount"] = analyst_ratings.get("sell", 0) + analyst_ratings.get(
            "strongSell", 0
        )
