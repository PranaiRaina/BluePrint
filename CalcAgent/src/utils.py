...
async def run_with_retry_stream(agent: Any, query: str, max_retries: int = 3, retry_delay: float = 1.0):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "tool_use_failed" in error_msg or "Failed to parse" in error_msg:
                if attempt < max_retries - 1:
                    yield {"type": "status", "content": f"Parsing error, retrying ({attempt+1}/{max_retries})..."}
                    await asyncio.sleep(retry_delay)
                    continue
            else:
                raise
